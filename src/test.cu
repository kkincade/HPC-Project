...
__global__ void EvaluateFSM(int* d_fsm, int* d_threadResultStates, int* d_inputs, int* d_startStates, size_t fsmPitch, size_t inputsPitch, int CHUNK_SIZE, int maxThreadIndex);
...
int main(int argc, char** argv) {
  ...
  if (argc > 1 && string(argv[1]) == "serial") {
    ...
    while (inputIndex < inputLength) {
      ...
      EvaluateFSM<<<NUM_BLOCKS, THREADS_PER_BLOCK>>>(d_fsm, d_threadResultStates, d_inputs, d_startStates, fsmPitch, inputsPitch, CHUNK_SIZE, threadIndex);
